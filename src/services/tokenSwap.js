...
// The devnet-deployed Token Swap program at TOKEN_SWAP_PROGRAM_ID is the
// classic/legacy build, whose Swap instruction takes ONE combined token
// program account (10 accounts total). The installed @solana/spl-
// token-swap package's own TokenSwap.swapInstruction() targets a newer
// on-chain interface added later for Token-2022 support (13-14 accounts,
// separate source/destination/pool token-program accounts) — sending
// that shape to the classic program misaligns every account after
// position 8 and fails with "the provided token program does not match
// the token program expected by the swap". This rebuilds the instruction
// by hand using the confirmed legacy layout instead. Pool creation
// (createInitSwapInstruction) is unaffected — that instruction's shape is
// identical between versions.
function legacySwapInstruction({
  tokenSwap,
  authority,
  ...
  // Instruction tag (1 byte) + amountIn (u64 LE) + minimumAmountOut (u64 LE).
  // Built by hand with writeBigUInt64LE for full 64-bit precision — plain
  // JS numbers (and buffer-layout's `nu64`) only safely hold 2^53.
  const data = Buffer.alloc(17);
  data.writeUInt8(1, 0); // Swap instruction tag
  ...
  if (hostFeeAccount) keys.push({ pubkey: hostFeeAccount, isSigner: false, isWritable: true });
...
export async function swapSolForToken({ walletProvider, tokenMint, solAmountIn, slippageBps = 100, onProgress }) {
  ...
  tx.add(
    legacySwapInstruction({
      tokenSwap: new PublicKey(config.swapAccount),
      authority: new PublicKey(config.authority),
      userTransferAuthority: payer,
      userSource: userWsolAta,
      poolSource: new PublicKey(config.vaultSol),
      poolDestination: new PublicKey(config.vaultToken),
      userDestination: userTokenAta,
      poolMint: new PublicKey(config.poolMint),
